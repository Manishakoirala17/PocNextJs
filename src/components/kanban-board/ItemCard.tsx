    ...
    return (
        <article draggable onDragStart={() => { setActiveCard(index) }} onDragEnd={() => { setActiveCard(null) }}>
            ...
            >
                ...
                {
                    isedited && <textarea name="task" id="task" className="outline-none border-none resize-none text-black p-3" onChange={(e) => { setUpdatedTask(e.target.value) }}>{taskDetail.name}</textarea> 
                }
                {

                    save && <p>{updatedTask}</p> 
